...
% Folder where to save/load the calculated results, as some calculations can take very long (up to a day):
...
do_this = 0; 
if do_this == 1
    cut_bg = 2; % 2 = subtract background,   0 = not subtract
    j_chan_all = 3:11;
    
    fig = 1:3;
    for j = fig; figure(j); clf; end % Clear 3 figures

    for j_chan = j_chan_all
        temp = Base_index(1,1,1) : Base_index(1,2,1); % Exp.1  baseline 1
        [ ~, ~] = DTOF_filter( squeeze(mean(Exp1_DTOF.TwoL(temp,j_chan,:),1)), Time_ns, bg_TwoL, cut_bg, 0, 0, {fig(1), 'red','norm'});
        temp = Base_index(2,1,1) : Base_index(2,2,1); % Exp.1  baseline 2
        [ ~, ~] = DTOF_filter( squeeze(mean(Exp1_DTOF.TwoL(temp,j_chan,:),1)), Time_ns, bg_TwoL, cut_bg, 0, 0, {fig(1), 'blue','norm'});
    end
    temp = ylim;
    temp(1) = temp(2) / 10^2; temp(2) = temp(2) / 10;
    plot([1 1]*bg_TwoL(1), temp,'Color','red','LineWidth',2)
    plot([1 1]*bg_TwoL(2), temp,'Color','red','LineWidth',2); 
    title('Exp. 1   Two-layered medium,  normalized')
    ...
    plot([1 1]*bg_TwoL(1), temp,'Color','red','LineWidth',2)
    plot([1 1]*bg_TwoL(2), temp,'Color','red','LineWidth',2); 
    title('Exp. 2   Two-layered medium,  normalized')

    for j_chan = j_chan_all
        temp = Base_index(1,1,3) : Base_index(1,2,3); % Exp.3  baseline 1
        [ ~, ~] = DTOF_filter( squeeze(mean(Exp3_DTOF.TwoL(temp,j_chan,:),1)), Time_ns, bg_TwoL, cut_bg, 0, 0, {fig(3),'norm'});
    ...
    plot([1 1]*bg_TwoL(1), temp,'Color','red','LineWidth',2)
    plot([1 1]*bg_TwoL(2), temp,'Color','red','LineWidth',2); 
    title('Exp. 3   Two-layered medium,  normalized')
    ...
    for j_chan = j_chan_all
        temp = Base_index(1,1,1) : Base_index(1,2,1); % Exp.1  baseline 1
        [ ~, ~] = DTOF_filter( squeeze(mean(Exp1_DTOF.Deep(temp,j_chan,:),1)), Time_ns, bg_Deep, cut_bg, 0, 0, {fig(1), 'red','norm'});
        temp = Base_index(2,1,1) : Base_index(2,2,1); % Exp.1  baseline 2
        [ ~, ~] = DTOF_filter( squeeze(mean(Exp1_DTOF.Deep(temp,j_chan,:),1)), Time_ns, bg_Deep, cut_bg, 0, 0, {fig(1), 'blue','norm'});
    end
    temp = ylim;
    temp(1) = temp(2) / 10^2; temp(2) = temp(2) / 10;
    plot([1 1]*bg_Deep(1), temp,'Color','red','LineWidth',2)
    plot([1 1]*bg_Deep(2), temp,'Color','red','LineWidth',2); 
    title('Exp. 3   Deep layer,  normalized')
    ...
    for j_chan = j_chan_all
        temp = Base_index(1,1,3) : Base_index(1,2,3); % Exp.3  baseline 1
        [ ~, ~] = DTOF_filter( squeeze(mean(Exp3_DTOF.Deep(temp,j_chan,:),1)), Time_ns, bg_Deep, cut_bg, 0, 0, {fig(3),'norm'});
        temp = Base_index(2,1,3) : Base_index(2,2,3); % Exp.3  baseline 2
        [ ~, ~] = DTOF_filter( squeeze(mean(Exp3_DTOF.Deep(temp,j_chan,:),1)), Time_ns, bg_Deep, cut_bg, 0, 0, {fig(3),'norm'});
    end
    temp = ylim;
...
% It takes little time to calculate moments, so no need to save/load them, unless for outputting somewhere else
...
j_chan_all = 1:14;
% j_chan_all = 3:11; % 3 to 11 were used for calculating concentrations for Publication 2023
% j_chan_all = 3;
...
for j_Exp = which_Exp % For which experiment to perform the calculation
% for j_Exp = 1 % For one experiment
    ...
    if isequal(1, j_Exp) % FIRST EXPERIMENT
        ...
        %                     temp_min = 1:1:90; % For a quick test, uncomment this and the lines below for figure(1000), this will do calculation only for temp_min and plot results here
...
% It takes little time to calculate concentrations, so no need to save/load them, unless for outputting somewhere else
