...
%     ylabels = {'\DeltaA','\Deltam_1','\DeltaV'};
    ...
    [ax, ~] = plotyyy(temp_X, zeros(size(temp_X))-10, temp_X, zeros(size(temp_X))-10,...
                temp_X, zeros(size(temp_X))-10, ylabels, xlimits_s, fig_num);
    ...
    for j_mom = 1:3
        ...
        % Exp. #1
        plot(temp_X, delMom.Exp1_TwoL_Blu(:,j_chan,j_mom)*ttt, 'o', 'Color', col{j_mom}, 'LineWidth',lw, 'MarkerSize', 10);
        plot(temp_X, delMom.Exp1_TwoL_Blu(:,j_chan,j_mom)*ttt, '--', 'Color', col{j_mom}, 'LineWidth',1, 'MarkerSize', 10);
    
        % Exp. #2
        if j_mom == 2; st = '+'; else; st = 'x'; end
        plot(temp_X_2, delMom.Exp2_TwoL_Blu(:,j_chan,j_mom)*ttt, st, 'Color', col{j_mom}, 'LineWidth',lw, 'MarkerSize', 14);
        plot(temp_X_2, delMom.Exp2_TwoL_Blu(:,j_chan,j_mom)*ttt, '--', 'Color', col{j_mom}, 'LineWidth',1, 'MarkerSize', 14);
        
        ylim(ylim_var(j_mom,:));
        
        if j_mom==1
            val_YTick = -0.5*5:0.5:1.5;
                set(gca,'YTick',val_YTick)
                set(gca,'YColor', [235 0 0]/255)
                grid on
        ...
        elseif j_mom==2
                set(gca,'YTick',round( val_YTick / ylim_var(1,1) * ylim_var(2,1) ))
                set(gca,'YColor', [0 0 204]/255)
                set(gca,'GridColor',[0 0 0])
                set(gca,'XTick',[])
        elseif j_mom==3
                set(gca,'YTick',round( val_YTick / ylim_var(1,1) * ylim_var(3,1) ))
                set(gca,'YColor', [0 190 0]/255)
                set(gca,'GridColor',[0 0 0])
                set(gca,'XTick',[])
        end
        set(gca,'FontSize',18)
...
%     plot_here = 1; % For plotting only '6a'.   2 = plot '6b'

% for j_chan = 1:14 % To run in a loop and see results for each wavelength
% for j_chan = 8 % For 2023 publication
