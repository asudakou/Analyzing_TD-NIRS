...
%
% Note, this script is good for seeing how to read the Molar Absorption Coefficients for HbO2 and Hb, 
% and the absorption of water, obtained from literature (i.e. downloaded)
% see "Where_downloaded.txt"


function [mua_nom] = Ink_Nominal_mua(varargin)
% This function can run witout input, in which case the wavelengths are manually inputted here and the spectra are plotted
% To run without input type in Command Window:  Ink_Nominal_mua;
...
%     wavelengths = 680:12.5:868; % 16 wavelengths, unit nm
    wavelengths = 650:2:900; % 16 wavelengths, unit nm
    do_plot = 1;
end
...
if do_plot == 1
    wavelengths_2 = 640:10:950;
    mua_Water_2 = GetExtinctions(wavelengths_2, 2); % [HbO Hb H2O lipid aa3]    [OD cm-1] for Water
end

if do_plot == 1
    figure(5); clf
    plot(wavelengths, Ext_coeff(:,2),'x-','Color','red','LineWidth',2,'MarkerSize',10) % Oxy
    hold on
    plot(wavelengths, Ext_coeff(:,3),'+-','Color','blue','LineWidth',2,'MarkerSize',10) % Deoxy
    grid on
    ylabel('Molar Absorption Coef. of HbO2 and Hb / [OD M^{-1} cm^{-1}]')
    ylim([900 6100])
    
    yyaxis right
    plot(mua_Water_all(:,1),mua_Water_all(:,2),'+','Color','green') % Water
    hold on
    plot(wavelengths_2, mua_Water_2(:,3),'x','Color','black','LineWidth',2,'MarkerSize',10) % Water, 2nd source
    ylabel('Absorption of Water / [OD cm^{-1}]')
    ylim([0 0.52])
...
% 4) For ink:  Absorption based on concentration of ink  -  see Eq. 5 in publication 2023
V_ink = zeros(21,2);
V_tot = zeros(21,2);
mua_nom = zeros(21,14,2);

for j_Exp = 1:2
    % Measured weights in gramms (Copy-pasted from Excel file)
    if j_Exp == 1
