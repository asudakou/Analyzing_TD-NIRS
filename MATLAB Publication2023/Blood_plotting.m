...
% bg_Deep = [0.49 0.92]; % Region for subtracting background noise
bg_TwoL = [0.49 0.92];
...
for j_chan = 3 % For 2023 publication
    ...
    for j_1 = 1:size(plot_index,1)
        [~, plot_index(j_1)] = min(abs(plot_minutes(j_1) - Exp1_Time_min)); % Exp 1
    end

    colcol = [0 00 204; 204 102 0; 0 180 0; 204 0 50]/255; % Colors
    
    for j = 1:4
        [ temp, ~ ] = DTOF_filter( squeeze(mean(Exp1_DTOF.TwoL(plot_index(j):plot_index(j)+10,j_chan,:),1)), Time_ns, bg_TwoL, 2, [0.25 0.03], 0);
    ...
    legend('Both min','Deep max','Sup max','Both max','25% and 3%')
end
end
