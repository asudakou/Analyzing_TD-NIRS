% DOWNLOADED FROM: https://www.mathworks.com/matlabcentral/fileexchange/1017-plotyyy


function [ax,hlines] = plotyyy(x1,y1,x2,y2,x3,y3,ylabels, xlimits_s, Fig)
...
cfig = get(gcf,'color');
pos = [0.1  0.1  0.7  0.8];
% offset = pos(3)/5.5; % 1.2
offset = pos(3)/7.5; % 1.142
offset = pos(3)/9; % 1.118
...
axes(ax(1)); xlim(xlimits_s);
axes(ax(2)); xlim(xlimits_s);
%Determine the proper x-limits for the third axes
limx1=get(ax(1),'xlim');
% limx3=[limx1(1)   limx1(1) + 1.2*(limx1(2)-limx1(1))];
% limx3=[limx1(1)   limx1(1) + 1.142*(limx1(2)-limx1(1))];
limx3=[limx1(1)   limx1(1) + 1.118*(limx1(2)-limx1(1))];
...
for j = 1:3 % Added by Aleh Sudakou 
    axes(ax(j))
    hold on
end